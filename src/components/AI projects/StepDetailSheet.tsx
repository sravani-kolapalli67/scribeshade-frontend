...
type DetailType =
  ...
  | { type: "purpose"; data: { purpose: string; businessPurpose: string; targetAudience: string } }
    ...
    <Sheet open={isOpen} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-xl w-full p-0 flex flex-col h-full border-l border-border/50 overflow-x-hidden">
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 no-scrollbar">
          <div className="py-6">
            {detail.type === "architecture" && <ArchitectureDetail data={detail.data} />}
            {detail.type === "flow" && <FlowDetail data={detail.data} />}
            {detail.type === "techStack" && <TechStackDetail data={detail.data} />}
            {detail.type === "collab" && <CollabDetail data={detail.data} />}
            {detail.type === "purpose" && <PurposeDetail data={detail.data} />}
            {detail.type === "dataDomains" && <DataDomainsDetail data={detail.data} />}
            {detail.type === "database" && <DatabaseDetail data={detail.data} />}
          </div>
        </div>
      </SheetContent>
    </Sheet>
...
function ArchitectureDetail({ data }: { data: SystemProject["architecture"] }) {
  return (
    <div className="space-y-6">
      <div>
        <div className="text-sm font-semibold mb-2">Pattern: {data.pattern}</div>
        <p className="text-sm text-muted-foreground leading-relaxed">{data.description}</p>
      </div>
      <div className="space-y-3">
        <h4 className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Layers</h4>
      ...
      {data.map((step, i) => (
        <div key={step.id} className="flex gap-4">
          <div className="flex flex-col items-center">
            ...
            {i !== data.length - 1 && <div className="w-px flex-1 bg-border my-1" />}
          </div>
          <div className="pb-3">
    ...
    <div className="space-y-5">
      {data.map((cat) => (
        <div key={cat.category}>
          <h4 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-2">{cat.category}</h4>
          <div className="space-y-2">
            {cat.items.map((item) => (
              <div key={item.name} className="flex items-center gap-3 p-3 rounded-lg bg-muted/30 border border-border/40">
...
function CollabDetail({ data }: { data: CollabTool[] }) {
  return (
    <div className="space-y-2.5">
      {data.map((tool) => (
        <div key={tool.name} className="flex items-center gap-3 p-3 rounded-lg bg-muted/30 border border-border/40">
...
function PurposeDetail({ data }: { data: { purpose: string; businessPurpose: string; targetAudience: string } }) {
  return (
    <div className="space-y-5">
      <div>
        <h4 className="text-xs font-bold uppercase tracking-widest text-brand mb-2">Purpose</h4>
        <p className="text-sm leading-relaxed">{data.purpose}</p>
      </div>
      <div className="p-4 rounded-xl bg-brand/5 border border-brand/10">
        <h4 className="text-xs font-bold uppercase tracking-widest text-brand mb-2">Business Goal</h4>
        <p className="text-sm leading-relaxed">{data.businessPurpose}</p>
      </div>
      <div>
        <h4 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-2">Target Audience</h4>
        <p className="text-sm leading-relaxed text-muted-foreground">{data.targetAudience}</p>
      </div>
    </div>
  );
}
...
function DataDomainsDetail({ data }: { data: DataDomain[] }) {
  return (
    <div className="space-y-4">
      {data.map((domain) => (
        <div key={domain.domain} className="p-4 rounded-xl border border-border/40">
          <div className="flex items-center justify-between mb-2">
            <span className="font-semibold text-sm">{domain.domain}</span>
            <Badge variant="outline" className="font-mono text-[10px]">{domain.size}</Badge>
          </div>
          <div className="h-2 rounded-full bg-muted overflow-hidden mb-2">
            <div className="h-full rounded-full" style={{ background: domain.color, width: "75%", opacity: 0.7 }} />
          </div>
...
function DatabaseDetail({ data }: { data: { dbType: string; tables: DatabaseTable[] } }) {
  return (
    <div className="space-y-4">
      ...
      {data.tables.map((table) => (
        <div key={table.name} className="rounded-xl border border-border/40 overflow-hidden">
          <div className="px-4 py-2.5 bg-muted/30 border-b border-border/30 font-semibold text-sm font-mono">
            {table.name}
          </div>
          <div className="p-3 space-y-1.5">
            {table.attributes.map((attr) => (
              <div key={attr.name} className="flex items-center gap-3 text-xs px-2 py-1.5 rounded bg-muted/20">
                <span className="font-mono font-semibold min-w-[90px]">{attr.name}</span>
                <span className="text-brand font-mono min-w-[100px]">{attr.type}</span>
                {attr.constraints && (
                  <span className="text-muted-foreground text-[10px]">{attr.constraints}</span>
                )}
              </div>
            ))}
          </div>
