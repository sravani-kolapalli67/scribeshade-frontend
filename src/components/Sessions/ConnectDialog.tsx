    ...
    <Dialog open={open} onOpenChange={(v) => !v && onCancel()}>
      <DialogContent className="sm:max-w-lg max-h-[calc(100dvh-2rem)] flex flex-col gap-0 border-none shadow-2xl rounded-3xl p-0 overflow-hidden bg-background" aria-describedby={undefined}>
        ...
        <div className="min-h-0 overflow-y-auto overscroll-contain px-7 pb-7 pt-3 space-y-5">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-2">
              {HOW_TO_CONNECT.map(({ icon: Icon, label }) => (
                <div key={label} className="flex flex-col items-center gap-1 group cursor-pointer" title={label}>
                ...
                <div className={"flex items-center justify-between px-3 py-2.5 rounded-xl border text-xs font-medium " + (lowBalance ? "bg-amber-50/60 border-amber-300/60" : "bg-muted/30 border-border/50")}>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-muted-foreground">
                      {maxBracket ? creditsPerHour + " cr/hour · first " + maxBracket.graceZoneMinutes + " min free" : "Free session"}
                    </span>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    {lowBalance && (
                      <Button size="sm" variant="ghost" className="h-7 px-2 text-[11px] font-bold text-amber-700 hover:text-amber-800 hover:bg-amber-100 rounded-lg border border-amber-300/50" onClick={() => setBuyCreditsOpen(true)}>
                        Top up
                      </Button>
                    )}
                  </div>
          ...
          <BuyCreditsDialog open={buyCreditsOpen} onOpenChange={setBuyCreditsOpen} onSuccess={refreshBalance} />
          <div className="flex items-center gap-3 pt-1">
            <Button variant="outline" onClick={onCancel} disabled={activating} className="gap-2 h-11 px-6 rounded-xl font-semibold border-border hover:bg-muted/50 transition-all shadow-sm">
            ...
            <Button
              ...
              disabled={activating}
              className={cn("flex-1 h-11 rounded-xl font-bold text-sm shadow-lg hover:shadow-xl active:scale-95 transition-all gap-2", "bg-[#0f172a] dark:bg-white text-white dark:text-black")}
            >
              {activating ? (
                <>
